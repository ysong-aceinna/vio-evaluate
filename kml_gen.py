...
def kml_gen(data_dir, lla, heading=None, name='pathgen', 
            color='ffff0000', max_points=None, gap=5):
    '''
    Generate .kml file contents from lla and heading.
    Args:
        data_dir: directory to save the .kml files.
        lla: [Lat, Lon, Alt], [rad, rad, meter], numpy array of size (n,3)
        heading: in unit of deg, numpy array of size (1,), should with same length with lla.
    ...
    cnt = lla.shape[0]
    if lla.shape[1] != 3 or cnt != heading.size:
        raise ValueError("please check lla and heading's size.")

    lla = lla.transpose(1,0)
    lla[0] *= R2D
    lla[1] *= R2D
    lla = lla.transpose(1,0)

    kml_file = os.path.join(data_dir, name+'.kml')
    f = open(kml_file, 'w+')
    f.truncate()
    # write header
